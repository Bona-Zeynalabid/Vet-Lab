...
const ParasitologyRequestSchema = new mongoose.Schema({
  caseId: { type: String, required: true, trim: true },
  doc: { type: String, required: true },
  dateRequested: { type: Date, default: Date.now },
  status: { type: String, enum: ['pending', 'completed'], default: 'pending' },

  // Same as Parasitology
  sample: {
...
}, { timestamps: true, collection: 'parasitology_requests' });

ParasitologyRequestSchema.index({ caseId: 1, status: 1 });

const ParasitologyRequest = mongoose.models.ParasitologyRequest || mongoose.model('ParasitologyRequest', ParasitologyRequestSchema);
export default ParasitologyRequest;