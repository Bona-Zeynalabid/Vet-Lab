...
const BacteriologyRequestSchema = new mongoose.Schema({
  caseId: { type: String, required: true, trim: true },
  doc: { type: String, required: true },             // requesting doctor
  dateRequested: { type: Date, default: Date.now },
  status: { type: String, enum: ['pending', 'completed'], default: 'pending' },

  // All the same fields as the Bacteriology lab model
  sample: {
...
}, { timestamps: true, collection: 'bacteriology_requests' });

BacteriologyRequestSchema.index({ caseId: 1, status: 1 });

const BacteriologyRequest = mongoose.models.BacteriologyRequest ||
  mongoose.model('BacteriologyRequest', BacteriologyRequestSchema);
export default BacteriologyRequest;