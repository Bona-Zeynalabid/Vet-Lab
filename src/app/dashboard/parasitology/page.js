...
import { casesApi, parasitologyApi, labRequestApi } from "@/lib/api";

export default function ParasitologyDashboardPage() {
  const router = useRouter();
  const [cases, setCases] = useState([]);
  const [parasitologyRecords, setParasitologyRecords] = useState([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [casesData, paraData, requestsData] = await Promise.all([
        casesApi.list({ lab: "parasitology" }),
        parasitologyApi.list(),
        labRequestApi.list({ lab: "parasitology", status: "pending" }),
      ]);
      setCases(casesData || []);
      setParasitologyRecords(paraData || []);
      ...
      <div className="border-b-2 border-slate-800 pb-3 flex flex-col sm:flex-row sm:items-end justify-between gap-2">
        <div>
          <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500 block">
            Diagnostic Laboratory / Parasitology Unit
          </span>
                  ...
                  <button
                    onClick={() => {
                      ...
                      const link = selectedDoc
                        ? `/forms/parasitology?caseId=${selectedCase.caseInfo?.caseNumber}&doc=${selectedDoc}`
                        : `/forms/parasitology?caseId=${selectedCase.caseInfo?.caseNumber}`;
                      router.push(link);
                    }}
